...
if N_dim == 1
    % only leave out ends
%     Kvp = [1, N_HF]';
    [~, max_idx] = max(xHF); [~, min_idx] = min(xHF);
    Kvp = [max_idx, min_idx]';
    loo_i = setdiff(1:N_HF, Kvp); 
    %loo_i = (2:N_HF-1)'; 
%     if isempty(loo_i); loo_i = N_HF; end
...
if N_fns == 1
    ...
    %[~, h_dist1_idx] = min(h_dist_avg_loo_RMSEs_1);
