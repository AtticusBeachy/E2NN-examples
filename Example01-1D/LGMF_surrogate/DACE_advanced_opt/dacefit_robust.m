function  [dmodel, perf] = dacefit_robust(S, Y, regr, corr, theta0, lob, upb)
...
%   [theta f fit perf] = boxmin(theta0, lob, upb, par);
  [theta f fit] = boxmin(theta0, lob, upb, par);
  if  isinf(f)
...
function  [THopt, fopt, fit] = boxmin(t0, lo, up, par)
% function  [t, f, fit, perf] = boxmin(t0, lo, up, par)
%BOXMIN  Minimize within box constraints
...
up = log10(up);
Ndim = length(up);
obj = @(THexp)objfunc(10.^THexp, par);


% global surrogate optimization
tic
options = optimoptions('surrogateopt', 'MaxFunctionEvaluations', 100*Ndim,...
                       'InitialPoints',t0);
[THexp_init_srgt, fopt_init_srgt] = surrogateopt(obj, lo, up, options);
time_surrogate = toc

% global patternsearch optimization
tic
options = optimoptions('patternsearch',...
                       'MaxFunctionEvaluations', 200*Ndim, ...
                       'MeshTolerance',1e-6,...
                       'UseVectorized',false,'UseCompletePoll',true,...
                       'UseParallel', false);
[THexp_init_ptrn, fopt_init_ptrn, exitflag] = patternsearch(obj,...
                                      THexp_init_srgt,[],[],[],[],lo,up,[], options);
time_pattern = toc

% use ptrn result if optimization didn't fail and better solution found
if exitflag ~= 2 && fopt_init_ptrn < fopt_init_srgt
    THexp_init = THexp_init_ptrn;
    fopt_init = fopt_init_ptrn;
    disp('Using Patternsearch result')
else
    THexp_init = THexp_init_srgt;
    fopt_init = fopt_init_srgt;
    disp('Using Surrogate Optimization result')
end


% local optimization
tic
options = optimoptions('fmincon','Algorithm','sqp','MaxIterations', Inf, 'MaxFunctionEvaluations',Ndim*1000,... 
