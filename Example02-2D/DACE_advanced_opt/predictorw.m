function  [y, or1, or2, dmse] = predictorw(x, dmodel)
...
% Last update August 26, 2002
  ...
  if  mx == 1  % one site only
      error(sprintf(' prediction on multiple locations not a single x'))
...
%     y = (dmodel.Ysc(1,:) + dmodel.Ysc(2,:) .* sy)';
    ...
    wgamma= wiR*(Y-F*wb)
%     wgamma= iR*(Y-F*wb)
    sy = f * wb + (wgamma'*inv(W)*r).'; %line: 126
%     sy = f * wb + (wgamma'*r).'; %line: 126 5/13 which one is it line 126
%     or 127?  ans.. line 126 is correct.
    % Predictor
...
%       or1 = repmat(dmodel.sigma2,mx,1) .* repmat((1 + colsum(u.^2) - colsum(rt.^2))',1,q);
      ...
      or1 = repmat(dmodel.wsigma2,mx,1) .* repmat((1 + colsum(v.^2) - colsum(wrt.^2))',1,q);
      
%       u = F'*wiR*iW*r-f';
%       uhat=u*inv(F'*wiR*F)*u'
%       rhat=(iW*r)'*wiR*(iW*r)
...
%  Look into this ///// commented on 0509 Friday
% %%%%%%%%%  sigma.....      
%        u = F'*wiR*inv(W)*r-f';
%       fR=inv(iR);
%       wfR=inv(W)*(inv(W)*fR)';
...
%       or1 = repmat(dmodel.wsigma2,mx,1) .* repmat((1 + suT' - srT')',1,q);
