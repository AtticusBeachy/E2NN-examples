function  [dmodel, perf] = dacefit(S, Y, regr, corr, theta0, lob, upb, MIC, useMIC)
...
%   [theta f fit perf] = boxmin(theta0, lob, upb, par);
  [theta f fit] = boxmin(theta0, lob, upb, par, MIC, useMIC);
  if  isinf(f)
...
obj = log(obj); %%%%%%%%%%%%%%%%%%%%% - AJB edit
% --------------------------------------------------------

function  [THopt, fopt, fit] = boxmin(t0, lo, up, par, MIC, useMIC)
% function  [t, f, fit, perf] = boxmin(t0, lo, up, par)
%BOXMIN  Minimize with positive box constraints

% Transform bounds to a log-scale so very small values get sufficient
% evaluations
lo = log10(lo);
up = log10(up);

% % set to SAMO paper value
% THopt = [1.56e-06, 1.39e-06, 1.27e-06, 4.00e-06, 1.22e-06, 2.18e-06, 5.78e-06, 7.32e-06, 2.32e-06, 1.13e-06];
% [fopt, fit] = objfunc(THopt, par);
% fopt

if useMIC
    % KMIC ND optimize (lambda is the auxiliary parameter to optimize)
    ncomp = size(MIC, 2);
    % objKMIC = @(lambda_exp) objfunc(sum(10.^lambda_exp.*MIC, 2), par); % WORKS!
    objKMIC = @(lambda_exp) objfunc(abs(sum(10.^lambda_exp.*MIC, 2)), par); % PROBABLY BETTER!
    options = optimoptions('surrogateopt', 'MaxFunctionEvaluations', 70);%200);%70);%200);%3000);
    lamopt_exp = surrogateopt(objKMIC, lo(1:ncomp), up(1:ncomp), options);
%     options = optimoptions('fmincon','Algorithm','sqp');
    options = optimoptions('fmincon','Algorithm','sqp','MaxIterations', Inf, 'MaxFunctionEvaluations', 5000,...
        'TolFun',1e-18,'TolX',1e-12);
    lamopt_exp = fmincon(objKMIC,lamopt_exp,[],[],[],[],lo(1:ncomp),up(1:ncomp),[],options);
    [fopt, fit] = objKMIC(lamopt_exp);
    lamopt = 10.^lamopt_exp
    THopt = abs( sum( lamopt.*MIC , 2) ) % abs only needed for probably better code
    fopt
else
    % conventional optimize
    options = optimoptions('surrogateopt', 'MaxFunctionEvaluations', 120);%500);%120);%200);%100);%500);
    [THexp_init, fopt_init] = surrogateopt(@(THexp)objfunc(10.^THexp, par), lo, up, options)
    
    options = optimoptions('fmincon','Algorithm','sqp','MaxIterations', Inf, 'MaxFunctionEvaluations', 5000,...
        'TolFun',1e-18,'TolX',1e-12);
    [THopt_exp, ~] = fmincon(@(THexp)objfunc(10.^THexp, par),THexp_init,[],[],[],[],lo,up,[],options);
    THopt = 10.^THopt_exp;
    [fopt, fit] = objfunc(THopt, par);
    THopt
    fopt
    if fopt_init < fopt
        warning('Fmincon made things worse!')
    end
end
...
% % KMIC 1D optimize (lambda is the auxiliary parameter to optimize)
% objKMIC = @(lambda_exp) objfunc(10.^lambda_exp*MIC, par);
% lamopt_exp = surrogateopt(objKMIC, lo(1), up(1));
% options = optimoptions('fmincon','Algorithm','sqp');
% lamopt_exp = fmincon(objKMIC,lamopt_exp,[],[],[],[],lo(1),up(1),[],options);
% [fopt, fit] = objKMIC(lamopt_exp);
% lamopt = 10.^lamopt_exp 
% THopt = lamopt*MIC
% fopt
